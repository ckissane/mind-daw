...
pub struct ClassifierPrediction {
    ...
    pub is_novel: bool,
    /// Raw distance to the best-matching class (lower = better match).
    pub novelty_score: f32,
}

/// Classify a single epoch against the trained classifier.
pub fn predict(epoch: &StimulusEpoch, classifier: &TrainedClassifier) -> ClassifierPrediction {
    let features = extract_features(epoch);
    predict_features(&features, classifier)
}
...
pub fn predict_features(features: &[f32], classifier: &TrainedClassifier) -> ClassifierPrediction {
    ...
    let novelty_score = 1.0 - best_score;
